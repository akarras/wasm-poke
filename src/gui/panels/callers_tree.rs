...
impl CallersTreePanel {
    ...
    fn subtree_contains_match(
        ...
        // Check children (callers)
        visited.insert(func_index);
        let has_match = if let Some(callers) = reverse_graph.get(&func_index) {
            callers.iter().any(|&caller| {
                Self::subtree_contains_match(caller, filter, module, reverse_graph, visited, depth + 1)
            })
        } else {
            false
        };
    ...
    fn render_tree_node(
        ...
        if !filter.is_empty() {
            ...
            let subtree_matches =
                Self::subtree_contains_match(func_index, filter, module, reverse_graph, &mut filter_visited, depth);
        ...
        let id = ui.make_persistent_id(("callers_tree_node", node_path.clone()));
            ...
            header_response.body(|ui: &mut egui::Ui| {
                if let Some(callers) = callers {
                    for (child_pos, &caller_index) in callers.iter().enumerate() {
                        ...
                        child_path.push((caller_index, child_pos));
